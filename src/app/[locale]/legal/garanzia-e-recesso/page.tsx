...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    ...
    alternates: buildAlternates(locale, "legal/garanzia-e-recesso"),
...
export default async function GaranziaPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const content = locale === "it" ? {
    ...
    sections: [
      ...
      { title: "Procedura di recesso", content: "Per esercitare il diritto di recesso, è necessario inviare una comunicazione scritta via email a info@doctor-haus.com o tramite PEC a montaggisrl@pec.it, utilizzando il modulo tipo di recesso disponibile su richiesta." },
    ...
    sections: [
      ...
      { title: "Withdrawal procedure", content: "To exercise the right of withdrawal, you must send a written communication via email to info@doctor-haus.com or via PEC to montaggisrl@pec.it, using the standard withdrawal form available upon request." },
