...
import { Mail, ArrowRight, AlertCircle, Lock, User, Eye, EyeOff } from "lucide-react";
import { setStoredSession } from "@/lib/customer-auth";
import { useRouter } from "next/navigation";

export default function AccediContent() {
  ...
  const checkEmail = async () => {
    ...
    try {
      ...
      if (result.type === "admin") {
        ...
        // No password set yet for this email — send them to set one rather than
        // granting a session on email alone (email ownership isn't verified here).
        localeRouter.push(`/registrati?email=${encodeURIComponent(email.trim())}`);
      }
    } catch {
